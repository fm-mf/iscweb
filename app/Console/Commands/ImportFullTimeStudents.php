...
class ImportFullTimeStudents extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'fullTimeStudents:add {fileName}';
     ...
     * @var string
     */
    protected $description = 'Import Full-time students from xml file and send them a registration email';
    /**
    ...
    public function handle()
    {
        ...
        foreach ($exchangeStudents as $exchangeStudent) {
            ...
            if ($this->createExchangeStudent($exchangeStudent)) {
                $importedCount++;
            } else {
                $alreadyInCount++;
            }
        ...
        $count = 0;
        foreach ($this->mailToStudents as $student) {
    ...
    private function createUser($data): User
    {
        ...
        try {
            ...
            $person->age = intval($data[$this->yearOfBirthKey]);
    ...
    private function createExchangeStudent($data): bool
    {
        ...
        $exchangeStudent->person->user->addRole(9);
        $this->info('Added role 9 (samoplatce) to id: ' . $exchangeStudent->id_user);
        $this->addSemesters($exchangeStudent, $data);
        $this->info('ExchangeStudent created');
        $this->logger->addLine($exchangeStudent);
        array_push($this->mailToStudents, $exchangeStudent);
        return true;
    }
