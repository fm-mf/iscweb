                ...
                <h3>{{ $exStudent->buddy->person->first_name .' '. $exStudent->buddy->person->last_name}}</h3>
                    @if($buddyRemovable)
                        <protectedbutton  url="{{ url('partak/users/buddies/'. $exStudent->buddy->id_user .'/remove/' .$exStudent->id_user) }}"
                                          protection-text="Remove buddy {{ $exStudent->buddy->person->first_name }} {{ $exStudent->buddy->person->last_name }}?"
                                          button-style="btn-danger"><span class="glyphicon glyphicon-remove up"></span> Remove</protectedbutton><br>@endif
